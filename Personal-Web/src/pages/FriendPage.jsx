    ...
    <div className="fq-body">
      <div className="fq-dash-two-col">
        ...
        <div className="fq-dash-left">
          ...
          {(() => {
            // Classify each friend by score trend.
            // Priority: regression over combined data (≥2 pts) → survey delta (≥2 surveys) → single survey tier
            const healthData = profiles.map(p => {
              const pSurveys = surveys
                .filter(s => s.profileId === p.id && s.total != null)
                .sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
              const latest = pSurveys[pSurveys.length - 1];
              if (!latest) return null; // no survey at all — skip

              // Try regression first
              const pts = buildRegressionPoints(surveys, journals, p.id);
              if (pts.length >= 2) {
                const reg = linearRegression(pts);
                if (reg) return { profile: p, slope: reg.slope * 30, score: latest.total, mode: 'reg' };
              }
              // Fall back: simple delta between latest and previous survey
              if (pSurveys.length >= 2) {
                const prev = pSurveys[pSurveys.length - 2];
                const delta = latest.total - prev.total;
                return { profile: p, slope: delta, score: latest.total, mode: 'delta' };
              }
              // Single survey — classify by score tier
              const tier = getScoreTier(latest.total);
              return { profile: p, slope: null, score: latest.total, tier: tier.tier, mode: 'single' };
            }).filter(Boolean);

            if (healthData.length === 0) return null;

            const rising  = healthData.filter(d => d.slope !== null && d.slope > 2).sort((a,b) => b.slope - a.slope);
            const falling = healthData.filter(d => d.slope !== null && d.slope < -2).sort((a,b) => a.slope - b.slope);
            const stable  = healthData.filter(d => d.slope === null || (d.slope >= -2 && d.slope <= 2));
                ...
                {[
                  ...
                  <div key={label} style={{ marginBottom:8 }}>
                    <div style={{ fontSize:10, fontWeight:700, color, letterSpacing:'0.06em', marginBottom:4, display:'flex', alignItems:'center', gap:4 }}>
                      ...
                      <span style={{ fontWeight:400, color:'var(--muted)' }}>({list.length})</span>
                    </div>
                      ...
                      {list.slice(0, 5).map(d => (
                        ...
                        >
                          ...
                          <span style={{ fontSize:10, fontWeight:700, color, flexShrink:0 }}>
                            {d.slope !== null
                              ? `${d.slope >= 0 ? '+' : ''}${d.slope.toFixed(0)}${d.mode === 'reg' ? t('/月','/mo') : ''}`
                              : `${d.score}`
                            }
                          </span>
                        </div>
                      ))}
                      {list.length > 5 && <div style={{ fontSize:10, color:'var(--dim)', paddingLeft:4 }}>+{list.length-5} {t('更多','more')}</div>}
                    </div>
                  </div>
                ))}
              </div>
